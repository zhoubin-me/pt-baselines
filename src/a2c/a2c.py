...
class A2CAgent(BaseAgent):
    def __init__(self, args):
        ...
        # self.rollouts = {
        #     "obs": torch.zeros(args.num_processes, args.nsteps, *self.envs.observation_space.shape).cuda(),
        #     "actions": torch.zeros(args.num_processes, args.nsteps).cuda(),
        #     "rewards": torch.zeros(args.num_processes, args.nsteps).cuda(),
        #     "dones": torch.zeros(args.num_processes, args.nsteps).cuda(),
        # }
        ...
        while steps < cfg.max_steps:
            ...
            with torch.no_grad():
                for step in range(cfg.nsteps):
                    v, pi = self.network(states / 255.0)
                    actions = Categorical(logits=pi).sample()

                    states, rewards, dones, infos = self.envs.step(actions)
                    rewards = self.reward_normalizer(rewards)
                    steps += cfg.num_envs
                    rollouts.append((states, actions, v, rewards, dones))
                ...
                # Compute R and GAE
                v_next, _, _ = self.network(states)
                ...
                for _, _, v, _, _, reward, done in reversed(rollouts):
                    ...
                    gae = gae * cfg.discount * cfg.gae_lambda + td_error
