...
from src.common.model import TD3MLP

class TD3Actor(AsyncActor):
    def __init__(self, cfg, lock, device_id):
        super(TD3Actor, self).__init__(cfg)
        self.lock = lock
...
class DDPGAgent(BaseAgent):
    def __init__(self, cfg):
        ...
        self.actor = TD3Actor(cfg, self.lock, cfg.device_id)
    ...
    def step(self):
        ...
        if self.total_steps > cfg.exploration_steps:
            ...
            with torch.no_grad():
                dist = Normal(self.target_network.p(next_state), self.network.p_log_std.expand_as(actions).exp())
            ...
            if self.total_steps % cfg.update_freq == 0:
                policy_loss = self.network.v1(torch.cat([state, self.network.p(states)], dim=1)).mean().neg()
